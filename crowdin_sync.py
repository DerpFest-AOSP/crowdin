...
def push_as_commit(config_files, base_path, path, name, branch, username):
    ...
    # Push commit
    try:
        repo.git.push(f'ssh://{username}@gerrit.pixysos.com:29418/{name}',
...
def main():
    ...
    base_path_env = f'CROWDIN_BASE_PATH'
    ...
    xml_main = load_xml(x=f'{base_path}/manifest/crowdin.xml')
    if xml_main is None:
        sys.exit(1)
